...
def InjectTransits_David(timeArray,fluxArray,KIC,DetrendingName,orbit_params,stellar_params,injection_type, injection_param_file = None):

	if (injection_type == "archive_sculpted" or injection_type == "archive_full_pileup" or injection_type == "archive_raw"):
		exoplanet_archive = Table.read('../Databases/exoplanet_archive.csv',delimiter=',',comment='#')
	
	if (injection_type == "manual_wata"):
		#Nov 2024 update: have the code read a .csv file so we can pre-sample the parameters
		if os.path.isfile('../Databases/'+injection_param_file):
			inj_params = Table.read('../Databases/'+injection_param_file)
			if int(KIC) not in inj_params['KIC']:
				print('KIC not in injection param file. Use manual injection param instead.')
				injection_type = 'manual' #have to use manual inj params if the KIC is not in the inj csv file
		else:
			print('Injection param file does not exist. Use manual injection param instead.')
			injection_type = 'manual' #also use the manual inj params if the file does not exist
	...
	# Get the limb darkening parameters
	# First, calculate logg
	logg_sun = 4.4374 # reference value used in the calculation
	...
	# The 0.00001 corresponds to the error on the value
	...
	ebin = orbit_params['e']
	#a_stabilitylimit=CBStabilityLimit(stellar_params['mA'],stellar_params['mB'],orbit_params['e'],abin) #How do I get ab?
	...
	if (injection_type == "random"):
		...
		minValue = 2.5
		maxValue = 4.0 # search bounds for 2.2 times binary apoapse to 4.1 times binary apoapse. This covers planets up to Kepler-453
		a_min = minValue*abin*(1+ebin) 
		a_max = maxValue*abin*(1+ebin)
		...
		# The only thing we take from the exoplanet archive is a random planet radius. The orbital parameters are always set to randomly near the pileup
		...
		# We randomly draw from the archive, and use that planet's semi-major axis as the true semi major axis, but only select the planet if it would be stable
		
		stablePlanetFound = False
		
		while (stablePlanetFound == False):
			random_archive_index = int(np.floor(np.random.uniform(0,len(exoplanet_archive))))
			
			_Pp = exoplanet_archive['pl_orbper'][random_archive_index] * days2sec
			_ap = PeriodToSemiMajorAxis(np.array([stellar_params['mA']+stellar_params['mB'],0]),np.array([_Pp]))
			if _ap > 2.5 * abin*(1+ebin) :
				print(str(_Pp/days2sec) + ' is stable around Pbin = ' + str(orbit_params['Pbin']/days2sec) + ' and ebin = ' + str(orbit_params['e']) )
				stablePlanetFound = True
				break
			else:
				print(str(_Pp/days2sec) + ' is UNSTABLE around Pbin = ' + str(orbit_params['Pbin']/days2sec) + ' and ebin = ' + str(orbit_params['e']) )
	...
	print('Injection of Rp = ' + str(_Rp/rEarth_m) + ' Rearth and Pp = ' + str(_Pp/days2sec) + ' days')
	
	
	z=stellar_params['mA'],stellar_params['mB'],orbit_params['Pbin'],orbit_params['e'],orbit_params['omega'],orbit_params['sep'],_Pp,_ep,_omegap,_thetap #orbit_params.sep is supposed to be theta? is that right??
	TT,TD,sigma_solutionOld,meanFlux_solution,stdOutOfTransit=Search_CreateTransitMask(z, stellar_params['rA'], stellar_params['rB'], timeArray,fluxArray,returnTransitTimes=True,meanTotalLightcurve=-27,plotting=False,mission='BLAH', ID='BLAH',SearchName='BLAH')

    #fix this file path later
	params_filename = '../Databases/' + 'planet_params.csv'
	np.savetxt(params_filename,np.transpose([stellar_params['mA']/mSun_kg,stellar_params['mB']/mSun_kg,stellar_params['rA']/rSun_m,stellar_params['rB']/rSun_m,orbit_params['Pbin']/days2sec,orbit_params['e'],orbit_params['omega'],_Rp/rEarth_m,_Pp/days2sec,_ep,_omegap,_thetap,random_archive_index]))

	flux_ratio=stellar_params['frat']

	numTransits = len(TT)
	depth = [] # used for storing the depths

	output_time = []
	output_duration = []
	output_impactparameter = []
	Tb=np.zeros(len(TT))




	# Planet parameters: David will look up the random range, but take ap = random between 2.5 and 3.5 abin
	# eccentricity: random uniform between 0 and 0.15 (range of known systems)
	# omega_p random between 0 and 2pi
	# theta_p random between 0 and 2pi

	# IMPORTANT: we want to save the planet parameters used to a file. particularly important since they are random so we want to remember what we put in, so when we go to recover the planet we can see if it's the right one.
	# Look at the LoadData function, which has a bit of code that saves the binary parameters to a text file. Essentially copy that but for these randomyl generated planet parameters.
	
	
	# randomly kick out some of the transits, based on a rough approximation of planets not always transiting due to the 3 body geometry and dynamics
	
	if (remove_random_transits == True):
	
		decider_1 = np.random.random()
	
		if decider_1 > 0.5:
			# 50% chance we kick out any
			cut_transits_index = int(np.floor(np.random.uniform(numTransits/2,numTransits)))
			TT = TT[0:cut_transits_index]
			TD = TD[0:cut_transits_index]
			
			print('old numTransits = ' + str(numTransits))
			numTransits = len(TT)
			print('new numTransits = ' + str(numTransits))
	


	for ii in range(0,len(TT)):
		# Create a MA light curve specifically for this transit
		params.per = _Pp/years2sec #orbits[1].P/years2sec
		params.t0 = 0.
		#Tb[ii] = 0.0
		params.inc = 90# - np.degrees(np.arcsin(Tb[ii]*p[0].r/orbits[1].a))
		params.ecc = 0. # planet and binary eccentricity will be accounted for later
		params.w = 0.
		params.u = [linParam,quadParam]
		params.rp = _Rp/stellar_params['rA']      #Does this want the radius of the planet for p[2].r? p[2].r/p[0].r
		params.a = 	_ap/stellar_params['rA']									#orbits[1].a/p[0].r
		
		
		# Calculate a rough duration
		duration_rough = _Pp*stellar_params['rA']/(np.pi*_ap) * (1+_Rp/stellar_params['rA']) /years2sec 	#orbits[1].P * p[0].r/(np.pi*orbits[1].a) * (1+p[2].r/p[0].r) / years2sec 
		
		# Create a time axis
		timeTransit_rough = np.linspace(-0.6*duration_rough,0.6*duration_rough,numDataPoints)
		
		# Evaluate the Batman model
		m = batman.TransitModel(params,timeTransit_rough)
		fluxTransit_rough = m.light_curve(params)
		
		# Now find the exact transit duration based on where MA is less than the first value
		...
		# Now define a new time axis that is scaled by the duration from the n-body (which does include the impact parameter)
		...
		# Now scale vertically by changing the transit depth according to the flux ratio
		depth_old = (_Rp/stellar_params['rA'])**2    #(p[2].r/p[0].r)**2.
		depth_new = depth_old * 1./(1.+flux_ratio)
		
		temp = 1.-fluxTransit_scaled
		temp = temp * depth_new/depth_old
		fluxTransit_scaled = 1.-temp
		...
		#fig = plt.figure()
		#ax = fig.add_subplot(111)
		#ax.scatter(timeTransit_rough*years2sec/hours2sec,fluxTransit_rough,color='r',label='fluxTransit_rough')
		#ax.scatter(timeTransit_precise*years2sec/hours2sec,fluxTransit_precise,color='b',label='fluxTransit_precise')
		#ax.scatter(timeTransit_scaled*years2sec/hours2sec,fluxTransit_scaled,color='g',label='fluxTransit_scaled')
		#ax.set_ylabel('Flux')
		#ax.set_xlabel('Time since mid transit point (hour)')
		#plt.show(block=False)
			
		
		# Print out some details
		
		print('Transit #' + str(ii) + ', time = ' + str(round(TT[ii]/days2sec - 55000,3)) + ', duration = ' + str(round(TD[ii]/hours2sec,3)) + ', depth = ' + str(round(depth[ii],8)) + ', b = ' + str(round(Tb[ii],3)))
	
		# Now we want to inject this into the light curve (if the duration is not 0)
		
		if (len(timeTransit_precise)>0):
			timeTransit_orig = timeInjected[(timeInjected > TT[ii] - 0.5*TD[ii]) & (timeInjected < TT[ii] + 0.5*TD[ii])]
			fluxTransit_orig = fluxInjected[(timeInjected > TT[ii] - 0.5*TD[ii]) & (timeInjected < TT[ii] + 0.5*TD[ii])]
		
			#print(timeTransit_orig - TT[ii])
			#print(timeTransit_scaled*years2sec)
			fluxTransit_injected = np.interp(timeTransit_orig - TT[ii],timeTransit_scaled*years2sec,fluxTransit_scaled)
			#print(fluxTransit_injected)
			...
			if (len(timeTransit_orig) > 0):
				# Only output injected transits if there is actually a light curve there to inject them into
				# This means that if there is a large gap in the light curve we don't count transits injected in the gap
				# This is useful for later when checking if an injected planet is recovered, as it would not be fair to check if an injected transit were found when in reality it occured during a gap in the data
	...
	#fig.savefig()
	fig.savefig('LightCurves/Injections/'+ DetrendingName + "/" + str(KIC) + '_injectedTransit.png', bbox_inches='tight')
	
	plt.show(block=False)
	
	# Save the list of times, durations and depths
	np.savetxt('LightCurves/Injections/' + DetrendingName + "/" + str(KIC)+ '_injectedTransitList.txt',np.transpose([np.array(output_time)/days2sec-55000,np.array(output_duration)/hours2sec,np.array(output_impactparameter)]))
	
	return timeInjected,fluxInjected


def InjectTransits_David2(
...
def InjectTransits_Hannah(timeArray, fluxArray, ID, mission, DetrendingName, orbit_params, stellar_params, base_dir=None):
    '''
    Functionality:
...
def create_detrending_diagnostic_report(
    *,
    detrending_name: str,
    mission: str,
    ID: str,
    is_kepler: bool,
    # Values returned from LoadDataTIC:
    override_bls: Optional[bool] = None,
    override_validation: Optional[bool] = None,
    # Values returned from iterative_bls_single_dip_search:
    bls_granular_settings: Optional[Dict] = None,
    bls_granular_presets: Optional[Dict] = None,
    # Optional: include a system label for the cover (e.g., "TIC260128333" or "KIC..."):
    system_label: Optional[str] = None,
    # Root of data tree (where LightCurves/, DiagnosticReports/ live).
    # If None, falls back to base_dir() which already honors STANLEY_BASE.
    base_root: Optional[Path] = None,
) -> Path:
    """
    Build a single PDF diagnostic report after detrending.

    Layout (relative to base_root):
      LightCurves/Data_Preparation/<DetrendingName>/
      LightCurves/Figures/<DetrendingName>/
      LightCurves/Processed/<DetrendingName>/...
      DiagnosticReports/<DetrendingName>_detrending_diagnostic.pdf
    """
    if system_label is None:
        system_label = f"{mission}{ID}"

    # ---------------- helpers ----------------
    def _collect_images(dirpath: Path) -> List[Path]:
        if not dirpath.exists() or not dirpath.is_dir():
            return []
        exts = {".png", ".jpg", ".jpeg", ".pdf", ".svg"}
        return sorted([p for p in dirpath.iterdir() if p.suffix.lower() in exts])

    def _read_binary_params_csv(csv_path: Path) -> dict:
        """Robustly read a params CSV. Supports key,value per row or header+single row."""
        if not csv_path.exists():
            return {}
        try:
            # Try DictReader first (header + one row of values)
            ...
            return {"_read_error": f"Failed to read {csv_path.name}: {e}"}

    def _read_timing_csv(csv_path: Path) -> dict:
        """Read timing CSV and try to surface a total runtime in seconds if present."""
        if not csv_path.exists():
            return {}
        try:
            with csv_path.open(newline="") as f:
            ...
            for r in rows:
                if len(r) == 2:
                    ...
                    if k.lower() in {
                        ...
                        "runtime_seconds",
                        "elapsed_seconds",
                    } and numeric_pattern.match(v):
                        timing["_total_seconds"] = float(v)
                elif len(r) == 1:
                    timing.setdefault("_rows", []).append(r[0].strip())
            if "_total_seconds" not in timing:
                for k, v in timing.items():
                    if isinstance(v, str) and "total" in k.lower():
                        try:
                            timing["_total_seconds"] = float(v)
                            break
                        except Exception:
    ...
    def _text_page(pdf: PdfPages, title: str, blocks: List[tuple[str, str]]) -> None:
        """
        Render a page (or multiple pages) of text blocks.
        Each block is (heading, content). Handles automatic page breaks.
        """
        ...
        fig.text(0.5, y, title, fontsize=18, weight="bold", ha="center", va="top")
        y -= 0.06

        def _new_page() -> tuple[plt.Figure, float]:
            pdf.savefig(fig)
            plt.close(fig)
            new_fig = plt.figure(figsize=(8.5, 11), dpi=150)
            new_ax = new_fig.add_axes([0, 0, 1, 1])
            new_ax.axis("off")
            new_y = 0.94
            return new_fig, new_y
        ...
        for heading, content in blocks:
            ...
            for para in paragraphs:
                ...
                for line in wrapped_lines:
                    if y < y_min:
                        fig, y = _new_page()
                    fig.text(0.05, y, line, fontsize=10, ha="left", va="top")
                    y -= line_step
    ...
    # ---------------- paths (robust) ----------------
    # If base_root is given, use it. Otherwise, infer root from the existing
    # LightCurves tree that the rest of the pipeline already uses.
    ...
    params_csv = processed_dir / f"{mission}_{ID}_{detrending_name}_binaryStartingParams.csv"
    timing_csv = processed_dir / f"{mission}_{ID}_{detrending_name}_LoadDataTICTiming.csv"

    # ---------------- gather content ----------------
    images: List[tuple[str, Path]] = []
    if not is_kepler:
        images.extend(
            (f"Data Preparation: {p.name}", p) for p in _collect_images(data_prep_dir)
        )
    images.extend((f"Figure: {p.name}", p) for p in _collect_images(figures_dir))

    params_dict = _read_binary_params_csv(params_csv) if params_csv.exists() else {}
    timing_dict = (
        _read_timing_csv(timing_csv)
        if (not is_kepler and timing_csv.exists())
        else {}
    )

    # ---------------- compose settings text ----------------
    cover_blocks: List[tuple[str, str]] = []
    title_label = system_label or detrending_name
    if not is_kepler:
        if override_bls is not None:
            cover_blocks.append(("override_bls", str(override_bls)))
        if override_validation is not None:
            cover_blocks.append(("override_validation", str(override_validation)))
        if bls_granular_settings:
            cover_blocks.append(
                (
                    "BLS granular settings",
                    json.dumps(bls_granular_settings, indent=2, ensure_ascii=False),
                )
            )
        if bls_granular_presets:
            cover_blocks.append(
                (
                    "BLS granular presets",
                    json.dumps(bls_granular_presets, indent=2, ensure_ascii=False),
                )
            )
        if timing_dict:
            if "_total_seconds" in timing_dict:
                cover_blocks.append(
                    (
                        "Detrending runtime (s)",
                        f"{timing_dict['_total_seconds']:.3f}",
                    )
                )
            cover_blocks.append(
                (
                    "Timing (raw)",
                    json.dumps(timing_dict, indent=2, ensure_ascii=False),
                )
            )
    else:
        cover_blocks.append(("Dataset", "Kepler (KIC)"))
        if system_label:
            cover_blocks.append(("System", system_label))

    if not cover_blocks:
        cover_blocks = [("", "No run settings provided.")]

    # ---------------- render PDF ----------------
    with PdfPages(out_pdf) as pdf:
        # COVER / SETTINGS PAGE
        _text_page(pdf, f"{title_label} — Detrending Diagnostic", cover_blocks)

        # FIGURE PAGES: 2 IMAGES PER PAGE
        if images:
            for i in range(0, len(images), 2):
                ...
                # TOP
                title1, img1 = pair[0]
                ax1 = fig.add_axes([0.05, 0.55, 0.90, 0.38])
                ax1.axis("off")
                fig.text(0.5, 0.96, title1, ha="center", va="top", fontsize=12)
                try:
                    if img1.suffix.lower() in {".pdf", ".svg"}:
                        ax1.text(
                            0.5,
                            0.5,
                            f"(Vector figure: {img1.name})",
                            ha="center",
                            va="center",
                            fontsize=11,
                        )
                    else:
                        ax1.imshow(plt.imread(str(img1)))
                except Exception as e:
                    ax1.text(
                        0.5,
                        0.5,
                        f"Failed to render {img1.name}\n{e}",
                        ha="center",
                        va="center",
                        fontsize=11,
                    )
                ...
                if len(pair) == 2:
                    title2, img2 = pair[1]
                    ax2 = fig.add_axes([0.05, 0.07, 0.90, 0.38])
                    ax2.axis("off")
                    fig.text(0.5, 0.48, title2, ha="center", va="top", fontsize=12)
                    try:
                        if img2.suffix.lower() in {".pdf", ".svg"}:
                            ax2.text(
                                0.5,
                                0.5,
                                f"(Vector figure: {img2.name})",
                                ha="center",
                                va="center",
                                fontsize=11,
                            )
                        else:
                            ax2.imshow(plt.imread(str(img2)))
                    except Exception as e:
                        ax2.text(
                            0.5,
                            0.5,
                            f"Failed to render {img2.name}\n{e}",
                            ha="center",
                            va="center",
                            fontsize=11,
                        )
            ...
            _text_page(pdf, "Figures", [("", "No figures found.")])

        # PARAMS PAGE
        params_blocks: List[tuple[str, str]] = []
        if params_dict:
            params_blocks.append(
                (
                    "Binary Starting Parameters (CSV)",
            ...
            params_blocks.append(
                (
                    "Binary Starting Parameters (CSV)",
                    "No parameters found or file missing.",
                )
            )
        if params_csv.exists():
            try:
                rel = str(params_csv.relative_to(root))
            except Exception:
                rel = str(params_csv)
            params_blocks.append(("Source file", rel))

        _text_page(pdf, "Binary Starting Parameters", params_blocks)

    return out_pdf


def create_search_analysis_diagnostic_report(
    *,
    search_name: str,
    mission: str,          # "TIC" or "KIC"
    ID: str,               # e.g., "260128333"
    system_label: Optional[str] = None,
    # Root of data tree; if None, uses base_dir().
    base_root: Optional[Path] = None,
) -> Path:
    """
    Build a Search & Analysis diagnostic PDF.

    Expects files in:
      <base_root>/PlanetSearchOutput/<search_name>/
        - {mission}_{ID}_SearchResults.txt
        - {mission}_{ID}_discoveredTransitList.txt
        - {mission}_{ID}_minutesTimeTaken.txt  (or .npy)
        - Any figures (png/jpg/jpeg/pdf/svg)

    Output PDF:
      <base_root>/DiagnosticReports/<search_name>_search_analysis_diagnostic.pdf
    """
    ...
    def _load_minutes(path: Path) -> Optional[float]:
        if not path.exists():
            return None
        try:
            if path.suffix == ".npy":
    ...
    def _text_page(pdf: PdfPages, title: str, blocks: List[tuple[str, str]]) -> None:
        ...
        ax.axis("off")
        y = 0.94
        plt.text(0.5, y, title, fontsize=18, weight="bold", ha="center", va="top")
        y -= 0.04
        wrap_w = 95
        for heading, content in blocks:
            if y < 0.08:
    ...
    def _image_page(pdf: PdfPages, img_path: Path, title: str = "") -> None:
        fig = plt.figure(figsize=(8.5, 11), dpi=150)
        ax = fig.add_axes([0, 0, 1, 1])
        ax.axis("off")
        if title:
            plt.text(0.5, 0.98, title, fontsize=12, ha="center", va="top")
        try:
            if img_path.suffix.lower() in {".pdf", ".svg"}:
                plt.text(
                    0.5,
                    0.5,
                    f"(Vector figure: {img_path.name})",
                    ha="center",
                    va="center",
                    fontsize=11,
                )
            ...
            plt.text(
                0.5,
                0.5,
                f"Failed to render {img_path.name}\n{e}",
                ha="center",
                va="center",
                fontsize=11,
            )
    ...
    sys_label = system_label or f"{mission}{ID}"

    results_txt = search_dir / f"{mission}_{ID}_SearchResults.txt"
    disc_txt = search_dir / f"{mission}_{ID}_discoveredTransitList.txt"

    # support either .txt or .npy for minutes
    minutes_path_txt = search_dir / f"{mission}_{ID}_minutesTimeTaken.txt"
    minutes_path_npy = search_dir / f"{mission}_{ID}_minutesTimeTaken.npy"
    if minutes_path_txt.exists():
        minutes_val = _load_minutes(minutes_path_txt)
    else:
        minutes_val = _load_minutes(minutes_path_npy)

    figures = _collect_images(search_dir)
    results_text = _read_text(results_txt)
    discovered_text = _read_text(disc_txt)
    ...
    if minutes_val is not None:
        cover_blocks.append(("Time taken (minutes)", f"{minutes_val:.3f}"))
    else:
        cover_blocks.append(("Time taken (minutes)", "N/A"))

    with PdfPages(out_pdf) as pdf:
        _text_page(pdf, f"{sys_label} — Search & Analysis Diagnostic", cover_blocks)

        if figures:
            for img in figures:
                _image_page(pdf, img, title=f"Figure: {img.name}")
        else:
            _text_page(pdf, "Figures", [("", "No figures found in the search directory.")])

        _text_page(pdf, f"Search Results — {results_txt.name}", [("", results_text)])
        _text_page(
            pdf,
            f"Discovered Transit List — {disc_txt.name}",
            [("", discovered_text)],
        )
